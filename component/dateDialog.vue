    ...
    <div class="picker-container">
      ...
      >
        ...
        <div 
          ...
          @mousedown="handleMouseDown(column.type, $event)"
        >
...
export default {
  ...
  methods: {
    ...
    handleMouseDown(type, event) {
      ...
      const scroller = this.$refs[`${type}Scroller`][0]
      this.startScrollTop = scroller.scrollTop
    ...
    handleMouseMove(event) {
      ...
      const scroller = this.$refs[`${this.currentType}Scroller`][0]
      const currentIndex = Math.round(this.startScrollTop / this.itemHeight)
      
      // 只有当移动距离超过 itemHeight/2 时才触发滚动
      if (Math.abs(deltaY) >= this.itemHeight / 2) {
        const targetIndex = Math.max(0, Math.min(
          currentIndex + direction,
          this[`${this.currentType}s`].length - 1
        ))
        
        this.scrollToIndex(this.currentType, targetIndex)
        // 更新起始位置,为下一次移动做准备
        this.startY = event.clientY
        this.startScrollTop = targetIndex * this.itemHeight
      }
    },
    ...
    handleMouseUp() {
      ...
      const scroller = this.$refs[`${this.currentType}Scroller`][0]
      const currentIndex = Math.round(scroller.scrollTop / this.itemHeight)
      this.scrollToIndex(this.currentType, currentIndex)
